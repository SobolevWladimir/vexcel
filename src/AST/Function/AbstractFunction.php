...
abstract class AbstractFunction implements Expression
{
    ...
    public function __construct(
      ...
      throw new SyntaxError(
        'Количество аргументов меньше чем ' . $this->getNumberArguments()." Функция:".$token->value, $token);
        }

        if (\count($args) > $this->getNumberArguments()) {
      throw new SyntaxError(
        'Количество аргументов болльше чем ' . $this->getNumberArguments()." Функция:".$token->value, $token);
        }
    }
