...
```
Функция getValueByIdentifier принимает идентификатор переменной  (по умолчанию идентификатор равен имени переменной. (подробнее)[## Хранение формул в БД.]) и возращяет значение. 
